...
;--------------------------------------------------------------------------
;	64byte buffer (for command return)

	extern	boot_rep	;;;res		BOOT_REP_SIZE
;--------------------------------------------------------------------------
;	64byte buffer (for command receives)
...
;;	LED Initialization of the port.
...
;	bcf		LED, LED_PIN1
	...
	rcall	clr_ram			; If the application is started suddenly , I keep a USB uninitialized .
...
#ifdef USE_JP_BOOTLOADER_EN
	;;
	;; When boot SW is OFF to jump to the address 0x800.
	;;
	btfsc	JP_BOOTLOADER_PORT, JP_BOOTLOADER_PIN	; skip if 0 (jumper installed or button pressed)
	bra	appl_reset_vector	; Run Application FW

#endif

	;;
	;; However, if there is no application writen, we should be back here
...
;TODO: whats the point of this bugfix? Why would we need to loop?
;/********************************************************************
;Bug Fix: May 14, 2007
;*********************************************************************
;;; bugfix1 : ACTVIF clear.
;;;	bcf	UIR, ACTVIF
L001:
	btfss	UIR, ACTVIF
	bra	usb_state_machine_actif_end
	bcf	UIR, ACTVIF
	bra 	L001
;;; bugfix1 : ACTVIF clear ends here.
...
;	HOST_TO_DEV (ep1Bo) <==boot_cmd here are commands sent from the host.
...
;	DEV_TO_HOST (ep1Bi)	==>boot_rep buffer for returning data.
...
;--------------------------------------------------------------------------
;	Set the address of the various USB descriptor in pSrc,
;	and set the size of the USB descriptor in the W register
;--------------------------------------------------------------------------
;	Argument  W     : desc_tab[] offset byte from the start.
;	Result    pSrc  : send to host PC descritor at above ROM address.
;		  Count : send to host PC BYTE SIZE of descriptor
;
	global	get_desc_tab
get_desc_tab
	; 3 times W.
	movwf	Count	; store W into Count.
	addwf	Count,W	; Count added to W.
	addwf	Count,W	; Count added to W again, so 3*W.
...
#if	0 ;This is cancelled out
	// hidmon control transfer only:
	movlw	LOW(boot_rep)
	movwf	pSrc
	movlw	HIGH(boot_rep)
	movwf	(pSrc + 1)
...
;--------	SET_REPORT
;Åö	Receive the HID Report.
;	Specifically, copy the HID Report to boot_cmd [] buffer.
;
...
usb_sm_HID_rq_cls_rq_end
	movwf	Count
	bcf		ctrl_trf_mem, _RAM		; Return data is on the buffer RAM.
...
;;led_on
#if	VECT
	;// 0x800 operation is flashing RC1 LED
#define	BLINK_LED_PIN LED_PIN1
#else
	;// 0x000 operation is flashing RC0 LED
